...
- 断线后不再永久掉线：`sio.wait()` 返回后自动重连（之前断线一次插件就静默挂掉，面板显示 connected: false 直到重启 AstrBot）。

## [0.3.4] - 2026-09-22

### 修复

- 房间聊天注入 QQ 桶时崩溃（`MessageChain object is not iterable`)：`abm.message` 改为纯组件列表，适配器才能正常处理——此前聊天回复根本不会进 AstrBot 管道和记忆桶。
- 注入的聊天正文以用户原话开头，场景上下文精简为尾随的 `[听歌场景: ...]` 括注,记忆桶里存的消息更像用户真实发言而不是整段场景描述。

## [0.3.3] - 2026-09-22

### 修复

- 明确 `gemini_endpoint` 与 `gemini_model` 的关系：端点含 `{model}` 占位符时才替换为模型名；端点已把模型写死在路径里（如中转服务）时按原样使用，`gemini_model` 不再产生误导。面板模型名字段加了说明。

## [0.3.2] - 2026-09-22

### 新增

- 状态面板新增"设置"页：可直接修改 Gemini 端点/模型/API Key、片段与提前分析节奏、人设 prompt、昵称、代理、会话桶（chat_session_id/chat_platform_id）与聊天回应开关。保存后立即热生效，并持久化到插件数据目录（dashboard_overrides.json），重启不丢。API Key 读取时脱敏，留空或保持脱敏值不会被覆盖。

## [0.3.1] - 2026-09-22

### 修复

- 修复 bot 蹲在自己创建的空房时不跟进用户房间的根因：房间列表里自己的房 userCount=1（bot 自己），旧逻辑只看列表第一个有人的房且恰好等于当前房就跳过；现在排除当前房后找真正有人的房。
- 房间发现日志输出各房间 userCount，便于排查。

## [0.3.0] - 2026-09-22

### 新增

- 状态面板：内置轻量 HTTP 服务（`dashboard_port`，默认 6385，`0` 关闭），打开即见当前连接状态、所在房间、正在听的歌曲/封面/进度条，以及最近弹幕与动态；`/api/status` 提供 JSON 快照。

## [0.2.3] - 2026-09-21

### 修复

- 歌曲收尾改为 fire-and-forget 注入会话桶记录（`chat_session_id` 桶），收尾弹幕固定发送，不再等模型回复——此前收尾消息会把回复直接发到绑定的 QQ 会话（意外副作用），且注入失败时连收尾都没有。
- 移除已无人使用的 `astrbot_reply`。
