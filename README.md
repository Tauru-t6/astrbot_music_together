...
- 周期性请求服务端权威时钟（`player:sync_request`），防止弹幕随时间漂移。
- 暂停时以人设口吻说一句暂停反应（每次暂停只发一次）。
- 房间没有真人时自动退出并冷却，只加入有真人的房间。
...
| 字段 | 默认值 | 说明 |
| --- | --- | --- |
| `segment_seconds` | `30` | 每个音频片段的长度，插件限制在 10 到 180 秒。 |
| `analyze_lead_seconds` | `25` | 在片段开始前多少秒提交分析。 |
